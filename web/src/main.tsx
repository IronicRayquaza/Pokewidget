...
import { canPlayCries, playCry, preloadCry } from './core/audio';
import { isDesktop, openWidgetWindow } from './desktop';
import { WidgetView } from './ui/WidgetView';
import { BackgroundPicker, PokemonPicker, SetPicker, TrainerPicker } from './ui/pickers';

type Panel = 'pokemon' | 'set' | 'trainer' | null;
...
function App() {
  ...
  useEffect(() => onWidgetsChanged(() => setWidgets(listWidgets())), []);
  ...
  return (
    <div class="wrap">
      <header class="top">
        ...
        <p>Animated Pokémon sprites on your desktop and in your browser.</p>
      </header>
      ...
      <div class="columns">
        <div>
              ...
              {widgets.map((widget) => (
                ...
                >
                  ...
                  <div class="who">
                    ...
                    <span class="caption">{catalogs.set(widget.config.setId)?.label ?? 'Unknown set'}</span>
                  </div>
                </div>
              ))}
          ...
          {!canPlayCries() && (
            <div class="card chalk">
              <p class="caption">
                This browser cannot play Ogg audio, so cries are silent here. Chrome, Firefox and
                the desktop app all play them.
              </p>
            </div>
          )}
        </div>
...
function Editor({
  ...
  return (
    <>
      <div class="card">
        <div class="stage" style={{ width: '100%', height: previewHeight + 24 }}>
          <WidgetView
            config={config}
            catalogs={catalogs}
            width={previewWidth}
        ...
        <div class="row" style={{ marginTop: 14 }}>
          ...
          <button
            class="primary"
            onClick={() => void openWidgetWindow(widget.id, previewWidth, previewHeight)}
          >
            {isDesktop() ? 'Put it on the desktop' : 'Open in its own window'}
          </button>
        </div>
